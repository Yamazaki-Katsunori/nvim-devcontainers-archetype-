...
if [[ ! -f "${REPO_ROOT}/.devcontainer/devcontainer.json" ]]; then
  echo "ERROR: ${REPO_ROOT}/.devcontainer/devcontainer.json not found" >&2
  exit 1
fi
...
# ================================================================================
# devcontainer down 処理 (devcontainer cli 側で未実装のため docker コマンドで対応)
# ================================================================================
echo "=== compose down (keep volumes) ==="
docker compose --env-file .devcontainer/.env -f .devcontainer/compose.yaml down --remove-orphans 2>&1 | tee -a "$LOG_FILE" || true
...
./scripts/devcontainers/clis/cli_ssh_inject.sh 2>&1 | tee -a "$LOG_FILE"

# ===================================
# neovim 設定ファイル群の権限更新処理 (rebuild 時のみ)
# ===================================
echo "=== nvim-devc chown vscode attach run ==="
...
' 2>&1 | tee -a "$LOG_FILE"
echo "=== nvim-devc chown vscode attach: done ==="
